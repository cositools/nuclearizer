/*
 * MNCTModuleEnergyCalibrationNonlinear.cxx
 *
 *
...
#ifdef ___CINT___
ClassImp(MNCTModuleEnergyCalibrationNonlinear)
#endif
...
MNCTModuleEnergyCalibrationNonlinear::MNCTModuleEnergyCalibrationNonlinear() : MNCTModule()
{
  ...
  m_Name = "Nonlinear Energy Calibration";
...
MNCTModuleEnergyCalibrationNonlinear::~MNCTModuleEnergyCalibrationNonlinear()
{
  // Delete this instance of MNCTModuleEnergyCalibrationNonlinear
}
...
bool MNCTModuleEnergyCalibrationNonlinear::Initialize()
{
  ...
  for (int DetectorNumber=0; DetectorNumber<10; DetectorNumber++) {
    ...
    MString FileName = (MString)std::getenv ("NUCLEARIZER_CAL")
      +"/energy_nonlinear_D"+ DetectorNumberString + ".csv";
	...
	if (Line.BeginsWith("#") == false) {
	  ...
	  double e_0, e_1, e_2, e_3, e_4, f_0, f_1, f_2;
	  //if (sscanf(Line.Data(), "%c,%d,%lf,%lf,%lf,%lf\n",
	  //	     &PosNeg,&StripNumber,&e_0,&e_1,&f_0,&f_1) == 6) {
	  if (sscanf(Line.Data(), "%c,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n",
		     &PosNeg,&StripNumber,&e_0,&e_1,&e_2,&e_3,&e_4,&f_0,&f_1,&f_2) == 10) {
	    ...
	    m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][2] = e_2;
	    m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][3] = e_3;
	    m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][4] = e_4;
	...
	if (m_IsCalibrationLoadedStrip[DetectorNumber][SideNumber][StripNumber-1] == false) {
	  ...
	  m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][2] = 0.;
	  m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][3] = 0.;
	  m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][4] = 0.;
	  m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][0] = m_DefaultFwhm;
    ...
    if (m_IsCalibrationLoaded[DetectorNumber] == true) {
      mout << "Nonlinear energy calibration for D" << DetectorNumber << " successfully loaded!" << endl;
    } else {
      mout << "***Warning: Unable to fully load nonlinear energy calibration for D" 
...
bool MNCTModuleEnergyCalibrationNonlinear::AnalyzeEvent(MNCTEvent* Event) 
{
  ...
  for (unsigned int i=0; i<n; i++) {
    ...
    double Energy = m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][0]
      ...
      + m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][2]*ADC*ADC
      + m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][3]
      *TMath::Exp(-TMath::Power(ADC/m_CalibrationCoefficients[DetectorNumber][SideNumber][StripNumber-1][4],3.));
    SH->SetEnergy(Energy);
    double EnergyFwhm = sqrt(m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][0]
			     *m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][0]
			     +m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][1]
			     *m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][1]*Energy
			     +m_FwhmCoefficients[DetectorNumber][SideNumber][StripNumber-1][2]
...
void MNCTModuleEnergyCalibrationNonlinear::ShowOptionsGUI()
{
  // Show the options GUI - or do nothing
}


// MNCTModuleEnergyCalibrationNonlinear.cxx: the end...
////////////////////////////////////////////////////////////////////////////////
