...
bool MModuleRevan::AnalyzeEvent(MReadOutAssembly* Event)
{
  ...
  //if (Event->IsBad() == true) {
    // Nothing to do here - we already have bad flags raised
  //  return true;
  //}
  ...
  // Convert the data to text and parse it again
  ostringstream Out;
  ...
  while (getline(In, Line)) {
    ...
    if (Line.empty() == false && Line.back() == '\r') Line.pop_back();
    RawEvent->ParseLine(Line.c_str(), 200);
  }
