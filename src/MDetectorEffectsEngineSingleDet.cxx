  ...
  while ((SimEvent = m_Reader->GetNextEvent(false)) != nullptr) {
    ...
    for (MDEEStripHit& Hit : MergedStripHits) {
      ...
      for (size_t i = 1; i < Hit.m_SubStripHits.size(); ++i) {
        ...
        if (Timing < LowestNoisedTiming)
          LowestNoisedTiming = Timing;
      }
    ...
    while (tr != MergedStripHits.end()) {
      ...
      // if ((*tr).m_Timing != 0){
      if ((*tr).m_ROE.IsLowVoltageStrip()) {
        xExists[DetID] = 1;
      } else if (!(*tr).m_ROE.IsLowVoltageStrip()) {
        yExists[DetID] = 1;
      }
      // }
      ++tr;
