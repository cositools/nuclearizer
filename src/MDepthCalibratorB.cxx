...
MDepthCalibratorB::MDepthCalibratorB()
{
	m_LookupTablesLoaded = false;
}

/*
bool MDepthCalibratorB::LoadLookupTables(MString FName)
{
	MFile F;
	if( F.Open(FName) == false ){
		cout << "MDepthCalibratorB: failed to open lookup table file..." << endl;
		m_LookupTablesLoaded = false;
		return false;
	} else {
		MString Line;
...
bool MDepthCalibratorB::PixelTableExists(int Pixel){
...
/*
bool MDepthCalibratorB::GetZ(int Pixel, int CTD, double& Z){
	if(m_PixelTables.count(Pixel)){
...
bool MDepthCalibratorB::GetZ(int Pixel, double t, double& z, double r){ 
	if(m_PixelTables.count(Pixel)){
