...
MTIRecord::MTIRecord(size_t length)
{
	m_Length = length;
}

bool MTIRecord::Add(long int t, int64_t PPS)
{
	bool Ret = false;
...
bool MTIRecord::AddCorrect(long int& t, int64_t& PPS)
{
	if(m_Record.size() < 2){
		m_Record[t] = PPS;
		if(m_Record.size() == 2){
...
bool MTIRecord::Get(long int t, long int& UTCSecond, int64_t& PPS){
	if(m_Record.size() < 2){
		PPS = 0;
		return false;
	} else {
