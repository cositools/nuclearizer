 ...
 *
 *
 * Copyright (C) by Ares Hernandez, Andreas Zoglauer.
 * All rights reserved.
 *
 *
...
bool MNCTAspectReconstruction::AddAspectFrame(vector<uint8_t> Frame)
{
  ...
  public:
    ...
    /*specified time (when the GetAspect, GetAspectGPS, and 
     G etAspectMagnetometer search functions fail to find an MNCTAspect         *
  ...
  {	
    ...
    /*
     I f you want to test this program for a dummy GPS data instead of dummy magnetome*ter
    ...
    /*	Packet PacketA;
     P acketA.GPS_or_magnetometer = 0;	                                           *
    ...
    /*heading saved into PacketA (using Alex's code that parses data from the Frame) as
     t he double known as "magnetic_heading." In the case of the magnetometer, you s*ee, the
    ...
    /*This here is the interesting part (only used if our data is from the magnetometer). Here, we 
     c reate a text file named "example1.txt" and write the longitude and latitude ont*o it. Then,
    ...
    /*Here is the fun part. Or perhaps, the confusing part. Here, we use rotation matrices to convert
     f rom heading, pitch, and roll to horizontal coordinates. These coordinates are a*zimuth and 
    ...
    PyRun_SimpleString(
      ...
      /*Note that the initial pointing vectors (ZYorXpointing0) for Z, Y, and X do not actually 
       p oint in the Z, Y, and X directions. This is because these pointing vectors are *based off 
      ...
      /*These are the heading, pitch, and roll rotation matrices, named Z, Y, and X. They are named
       a s such because of the definition of heading, pitch, and roll. "Heading" is defi*ned as the
      ...
      /*Please note, universal pointing directions are specified using heading, pitch and roll by performing
       3 intrinsic rotations in a specific order (remember: rotation matrices in more t*han 2 dimensions do
      ...
      /*Here is where the action happens. Here we apply our ZYX matrix to our pointing vectors. These rotated
       v ectors are called ZYorXpointing1. Finally, we convert these vectors from their *form in the GPS's coordinate
      ...
      /*Note that the initial pointing vectors (ZYorXpointing0) for Z, Y, and X do not actually 
       p oint in the Z, Y, and X directions once again. This is because, just like with *the GPS,
      ...
      /*These are the same heading, pitch, and roll rotation matrices, named Z, Y, and X, once again.
       T hey are exactly the same as before.*/
       ...
       /*Here is where the action happens, just as before. Here we apply our ZYX matrix to our pointing vectors. 
        T hese rotated vectors are called ZYorXpointing1 again. Finally, we convert these* vectors from their form
    ...
    /*Alright, now that we've used heading, pitch, and roll from the GPS or magnetometer to find the
     e quatorial coordinates (azimuth and what pyephem calls "altitude") we need to re*ad them off of the
    ...
    /*Here comes the tricky part. Again we will make use of python in the form of PyRunSimpleString.
     W hat we do here we get all of the necessary inputs we need in order to find the *galactic
    ...
    PyRun_SimpleString(
      ...
      /*Now, we use pyephem to create an observer object that we will call COSI. We feed COSI the 
       g eographic longituude and latitude, what it calls the "elevation", the date and *time, and the
  ...
  /*Now we're really done, right? Nope. Next, we must check with the previous batch of old aspect data 
   w e have and see if it is too different from the new data. If it is, we flag our *data as untrustworthy.
  ...
  for(unsigned int i = 0; i < m_Aspects.size(); i++){
    ...
    /*First things first. We retrieve our old GPS aspect data (if we are checking if a batch of data from
     t he magnetometer needs a flag, this old data will just go unused) using the GetP*reviousGPS() function.
    ...
    /*Since we had to create a new for loop, we must retrieve the aspect information for each and every 
     M NCTAspect object we inspect to see if it needs a flag. We do so below: */
     ...
     /*Okay, now we retrieve our old magnetometer aspect data (if we are checking if a batch of data from
      t he GPS needs a flag, this old data will just go unused) using the GetPreviousMa*gnetometer() function.
      The way this function works will be described later.*/
...
/*Okay, first off, we see the "GetAspect()" function. This function behaves like the GetAspectGPS()
 f unction (more on that function later) except that if it is unable to find good *GPS data for a given
...
/*This function, GetPreviousGPS(), was not created for the purposes of being used by people as it will 
 g ive you the previous GPS aspect data even it is bad data. The purpose of this f*unction is to be used
