    ...
    try:
        with open(fname, 'r') as f:
            for line in f:
                ...
                if not side in self.sides: self.sides.append(side)
        ...
        print("Strip Map File \"{}\" is not formatted correctly: {}".format(fname, str(e)))
        return
    ...
    for asic in range(self.nAsic):
        for ch in range(self.nAsicCh):
            if (asic, ch) not in temp_stripmap:
                error = True
                print("Strip Map File \"{}\" is not formatted correctly: Does not contain entry for ASIC {}, Ch{}".format(fname, asic, ch))
