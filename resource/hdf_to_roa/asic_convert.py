...
class Hdf:
    ...
    def __init__(self, filename: str, mode: str = "w") -> None:
        ...
        self.mode = mode
        self._header = np.zeros(
            1,
            np.dtype(
                [("BufferType", np.uint32), ("UTC", np.uint64), ("Index", np.uint64)]
            ),
        )
    ...
    def create_table(self, name, dtype, title):
        return self._hdf_file.create_table(
            self.root, name, dtype, title=title, filters=self.filters
        )
    ...
    def _open(self) -> None:
        ...
        if "w" in self.mode:
            self.buffers = self._hdf_file.create_vlarray(
                self.root,
                "Buffers",
            ...
            self.headers = self.create_table(
                "Headers", self._header.dtype, title="Buffer header data"
            )
        else:
...
class AsicConvert:
    ...
    def glob(self, file_glob):
        ...
        if len(file_glob) == 0:
            ...
            f_glob = sorted([item for name in file_glob for item in glob(name)])
    ...
    def parse_buffer(self, data):
        ...
        oscillator = np.right_shift(
            (osc_chan & 0xFFF0), 4, dtype=("uint32")
        ...
        np_array = np.core.records.fromarrays(
            [asic, channel, adc, tac, oscillator, headers, dt], dtype=self.dtype
        )
