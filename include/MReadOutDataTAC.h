...
#ifndef __MReadOutDataTAC__
#define __MReadOutDataTAC__
...
class MReadOutDataTAC : public MReadOutData
{
  // public interface:
 public:
  ...
  MReadOutDataTAC(MReadOutData* Data);
  //! Simple default destructor
  virtual ~MReadOutDataTAC();
  ...
  //! Set the timing
  void SetTAC(unsigned int TAC) { m_TAC = TAC; }
  //! Get the timing
  unsigned int GetTAC() const { return m_TAC; }

  //! Clone this data element - the returned element must be deleted
  virtual MReadOutDataTAC* Clone() const;
...
#ifdef ___CLING___
 public:
  ClassDef(MReadOutDataTAC, 0) // no description
#endif
