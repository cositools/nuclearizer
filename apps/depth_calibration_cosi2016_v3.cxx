...
int main(int argc, char** argv)
{
	...
	if( options->ProcessSimData ){ //read in simulation data
		...
		while ((simEvent = Reader->GetNextEvent()) != 0) {
			...
			for(size_t i = 0; i < simEvent->GetNHTs(); ++i){
				MSimHT* HT = simEvent->GetHTAt(i);
				if(EnergyFilter(HT->GetEnergy(), options->EnergyWindows)){
					...
					if(DetectorName.BeginsWith("Detector")){
						...
						MVector Vec = VS->GetPositionInSensitiveVolume();
						double Depth = Vec.GetZ();
				...
				//		int YStrip = 38 - (GP.GetYGrid() + 1);
						...
						//fill the division histograms
						if(DivisionHistograms.count(DivisionCode) == 1){
							DivisionHistograms[DivisionCode]->Fill(Depth);
						} else {
							char name[32]; snprintf(name, sizeof name, "divdepth_%d", DivisionCode);
							TH1D* H = new TH1D(name,name,100,0.0,DetectorThicknesses[DetectorID]);
							H->Fill(Depth);
							DivisionHistograms[DivisionCode] = H;
						}

						//fill the division CTD templates
						double Timing = m_DepthCalibrator->GetSpline(DetectorID,true)->Eval(Depth);
						double Noise = RNG.Gaus(0,12.5/2.3548);
						if(DivisionCTDs.count(DivisionCode) == 1){
							DivisionCTDs[DivisionCode]->Fill(Timing+Noise);
						} else {
							char name[32]; snprintf(name,sizeof name, "divctd_%d", DivisionCode);
							TH1D* H = new TH1D(name, name, 240, -300.0, +300.0);
							DivisionCTDs[DivisionCode] = H;
							H->Fill(Timing+Noise);
						}
					}
				}
...
int PixelCodeToDivision(int PixelCode, int Divisions){
	...
	if(xd >= Divisions) xd = Divisions - 1;
	int yd = y/(36/Divisions);
	if(yd >= Divisions) yd = Divisions - 1;
