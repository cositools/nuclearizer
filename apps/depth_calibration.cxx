...
std::map<int, TSpline3*>* MakeDepthSplines(MString fname, bool invert){
	//when invert flag is set to true, the splines returned are CTD->Depth
	std::map<int, TSpline3*>* SplineMap = new std::map<int, TSpline3*>();
	MFile F; F.Open(fname);
...
bool GetDepthSplines(MString fname, std::map<int, TSpline3*>& SplineMap, bool invert){
	//when invert flag is set to true, the splines returned are CTD->Depth
	MFile F; F.Open(fname);
	vector<double> xvec, yvec;
	MString line;
	...
	while( F.ReadLine(line) ){
		if( line.Length() != 0 ){
			if( line.BeginsWith("#") ){
				vector<MString> tokens = line.Tokenize(" ");
				NewDetID = tokens[1].ToInt();
				if( xvec.size() > 0 ) AddSpline(xvec, yvec, SplineMap, DetID);
				DetID = NewDetID;
			} else {
				vector<MString> tokens = line.Tokenize(" ");
				xvec.push_back(tokens[0].ToDouble()); yvec.push_back(tokens[1].ToDouble());
