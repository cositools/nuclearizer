...
bool UTNEndToEnd_406_1::CompareOutputToReference(const MString& OutputFile, const MString& ReferenceFile)
{
  ...
  Passed = EvaluateTrue("End-to-end test 406-1", "open output file",
                        "The generated output file can be opened for comparison",
                        Output.is_open()) && Passed;

  ifstream Reference(ReferenceFile.Data());
  Passed = EvaluateTrue("End-to-end test 406-1", "open reference file",
  ...
  while (true) {
    ...
    if (HasOutput == false || HasReference == false) {
      Passed = Evaluate("End-to-end test 406-1", "line count",
    ...
    if (OutputLine != ReferenceLine) {
      Passed = Evaluate("End-to-end test 406-1", MString("line ") + Line,
